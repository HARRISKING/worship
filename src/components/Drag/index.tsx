...
interface IDragProps {
  ...
  onScroll: (params: { scrollTop: number }) => void; // onScroll 事件
  onSortEnd: (list: List) => void; // drag 结束事件
  onChange: (list: List) => void; // 改变事件
  onItemClick?: (params: { key: number; data: List }) => void; // 点击单个事件
  itemWidth: number | string; // 单个宽度
  renderItem?: (v) => React.ReactNode;
}

const Drag: React.FunctionComponent<IDragProps> = ({
  list = [],
  columns = 1,
  scrollTop = 0,
  onScroll,
  ...
  const [itemDom, setItemDom] = useState({
    ...
    left: 0,
    top: 0,
  }); // 每一项 item 的 dom 信息，由于大小一样，所以只存储一个
  const [itemWrapDom, setItemWrapDom] = useState({
    width: 0,
    height: 0,
    left: 0,
    top: 0,
  ...
  const [listData, setListData] = useState<any>([]); // 渲染数据
  ...
  const touchMove = (e) => {
    ...
    if (
      isFixed(endKey) ||
      IsOutRange(curX, columns, curY, rows, endKey, listData.length)
    )
      return;
    // 防止拖拽过程中发生乱序问题
  ...
  const initDom = (_listData) => {
    const {
      windowWidth: sysWindowWidth,
      windowHeight: sysWindowHeight,
  ...
  return (
    <View className="drag-wrapper" style={`height: ${itemWrapHeight}px`}>
      {listData.map((v, index) => (
        <View
          className={`drag-item ${
            itemTransiton && cur !== index ? 'itemTransition' : ''
          }`}
          key={v.dragId}
          data-key={v.sortKey}
          data-index={index}
          style={`transform: translate3d(${
            index === cur ? `${tranX}px` : v.tranX
          }, ${index === cur ? `${tranY}px` : v.tranY}, 0);width: ${
            itemWidth || `calc((100% - ${8 * columns}px) / ${columns})`
          }; z-index: ${index === cur ? curZ : ''}`}
          onClick={() => itemClick(v)}
        ...
        >
          <View
            className={`drag-item__info ${cur === index ? 'drag-active' : ''}`}
          >
            {renderItem ? (
              renderItem(v)
            ) : (
              <View className="drag-item__info-item">
                <Image src={v.data.url} />
                {!draging && (
                  <View
                    className="drag-item__info-close"

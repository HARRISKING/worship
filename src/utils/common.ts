...
export const getSystemInfo = (): AdvantagedSystemInfo => {
  ...
  if (!info) {
    ...
    if (/unknown/.test(model) || /SE/.test(model)) {
      ...
      advantagedSystemInfo?.statusBarHeight &&
      advantagedSystemInfo?.statusBarHeight > 30
    ) {
...
// 脱敏信息展示
export const dealSensitiveInfo = (
  info: string,
  retain: number,
  onlyCenter?: boolean,
): string => {
  if (info && retain) {
    ...
    const sensitive = '*';
    return onlyCenter
      ? `${info.slice(0, retain)}${sensitive.repeat(
          length - 2 * retain,
        )}${info.slice(-retain)}`
      : `${info.slice(0, retain)}${sensitive.repeat(length - retain)}`;
  } else {
    return '';
  }
};
 ...
 * @param initUser
 */
export const dealUserAttributes = (
  attributes: any[],
  initUser?: { [key: string]: any },
) => {
  const user: { [key: string]: any } = initUser || {};
  attributes?.forEach((item) => {
    user[item.attributeKey] = ['groups', 'labels', 'spaces', 'photos'].includes(
      item.attributeKey,
    )
 ...
 * @param key
 */
export const getAttributeByKey = (attributes: any[], key) => {
  const user: { [key: string]: any } = dealUserAttributes(attributes) || {};
  return user?.[key];
};
